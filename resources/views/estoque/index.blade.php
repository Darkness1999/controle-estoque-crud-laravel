        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100" x-data="{ tipo: '{{ old('tipo', 'entrada') }}' }">
                    ...
                    <div class="mb-4">
                        <label for="barcode_search" class="block font-medium text-sm">Scanear Código de Barras ou SKU</label>
                            ...
                            <div>
                                ...
                                <select id="product_variation_id" name="product_variation_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500" required>
                                    ...
                                    @foreach ($variations as $variation)
                                        <option value="{{ $variation->id }}" data-sku="{{ $variation->sku }}" data-barcode="{{ $variation->produto?->codigo_barras }}" @if(old('product_variation_id') == $variation->id) selected @endif>
                                            {{ $variation->produto?->nome ?? 'Produto Removido' }} - @foreach($variation->attributeValues as $value){{$value->valor}}@if(!$loop->last), @endif @endforeach (SKU: {{ $variation->sku }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div x-show="tipo === 'entrada'" class="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <input id="lote" name="lote" type="text" value="{{ old('lote') }}" :disabled="tipo !== 'entrada'" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500">
                                </div>
                                <div>
                                    <label for="data_validade" class="block font-medium text-sm">Data de Validade</label>
                                    <input id="data_validade" name="data_validade" type="date" value="{{ old('data_validade') }}" :disabled="tipo !== 'entrada'" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500">
                                </div>
                            </div>
                        </div>
            ...
            barcodeInput.addEventListener('keydown', function (event) {
                if (event.key === 'Enter') {
                    ...
                    for (let i = 0; i < variationSelect.options.length; i++) {
                        ...
                        if (option.dataset.sku === scannedValue || (option.dataset.barcode && option.dataset.barcode === scannedValue)) {
